...
- job_name: otel
  static_configs:
  - targets:
    - '{{ range nomadService "otelcol-prometheus" }}{{ .Address }}:{{ .Port }}{{ end }}'
- job_name: otel-collector
  static_configs:
  - targets:
    - '{{ range nomadService "otelcol-metrics" }}{{ .Address }}:{{ .Port }}{{ end }}'
...
EOH
     ...
     service {
      provider = "nomad"
        ...
        port     = "prometheus_ui"
        tags = [
          "traefik.http.routers.prometheus.rule=Host(`prometheus.demo`)",
